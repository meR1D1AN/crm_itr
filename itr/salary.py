...
def _calculate_schedule_2_2(day, month_days, monthly_salary, customer):
    """Расчет для графика 2/2"""
    start_day = customer.start_work.day
    is_odd_start = start_day % 2 == 1  # Нечетный день начала работы
    ...
    if is_odd_start:
        ...
        if month_days == 30:
            return monthly_salary / 15  # Оба набора дней дают /15
        elif month_days == 31:
            if day in {1, 4, 5, 8, 9, 12, 13, 16, 17, 20, 21, 24, 25, 28, 29}:
                return monthly_salary / 15
            return monthly_salary / 16
...
def _calculate_schedule_1_3(day, month_days, monthly_salary):
    """Расчет для графика 1/3"""
    if month_days == 31:
        return monthly_salary / 7 if day % 4 == 0 else monthly_salary / 8
    elif month_days == 30:
        return monthly_salary / 7 if (day - 1) % 4 < 2 else monthly_salary / 8
    elif month_days == 28:
        return monthly_salary / 7
    return 0


def _calculate_working_days(year, month, month_days, weekend_day=5):
    """Подсчет рабочих дней в месяце"""
    return sum(1 for day in range(1, month_days + 1) if calendar.weekday(year, month, day) < weekend_day)
...
def calculate_daily_salary(day, month_days, monthly_salary, work_schedule, customer, year=None, month=None):
    ...
    """
    if work_schedule == "2/2":
        return _calculate_schedule_2_2(day, month_days, monthly_salary, customer)

    elif work_schedule == "1/3":
        return _calculate_schedule_1_3(day, month_days, monthly_salary)

    elif work_schedule in ["5/2", "6/1"]:
        if year is not None and month is not None:
            weekend_day = 6 if work_schedule == "6/1" else 5
            working_days = _calculate_working_days(year, month, month_days, weekend_day)
            return monthly_salary / working_days if working_days > 0 else 0
